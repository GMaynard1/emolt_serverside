...
CREATE TABLE `VESSEL_STATUS`(
  ...
  `REPORT_TYPE` set('DATA','STATUS','MANUAL') NOT NULL COMMENT 'The type of status report. Data means that data were transmitted. Status means an automated status report. Manual means the report was triggered manually by an end user',
  ...
  `UPTIME` int NOT NULL COMMENT 'The number of minutes the system has been powered on',
